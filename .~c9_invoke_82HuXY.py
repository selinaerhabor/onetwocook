...
def manage_creations():
    ...
    alert = 'Here are the sections below'
    ...
    page_title = page_title, alert = alert)
...
def delete_cuisine(cuisine_id, cuisine_type):
    cuisine = mongo.db.cuisines.find({'_id': ObjectId(cuisine_id)})
    recipes = mongo.db.recipes.find({"cuisine_type": cuisine_type})
    if recipes :
	    alert = "You can't delete this cuisine as it has multiple recipes associated with it"
	    return render_template('managecreations.html', alert = alert)
